...
import {
  findRanges,
  getIndexOfSubstringEnd,
  getTextPieceFromTextRuns,
  LCSEndsWith2ndStrSubstring,
...
async function main() {
  ...
  if (!args[2]) {
    console.log("pass office file path as argument")
  }
  const filePath = args[2] ?? "input/document.docx";
  const fileName = filePath.split('/').pop()
  // unzip office file
  unzipOfficeFile(fileName);
  const documentXmlFilePath = `extracted/${fileName}/word/document.xml`;
  let documentXml = fs.readFileSync(documentXmlFilePath, 'utf8');

  const searchTextLength = searchText.length;
  const jsObject = await stringToObject(documentXml);
  ...
  fs.writeFileSync(documentXmlFilePath, objectToString(jsObject));
  // zip file
  zipOfficeFile(fileName);
}
